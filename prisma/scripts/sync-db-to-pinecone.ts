...
// 1. Setup Pinecone client and cleaning functions
...
for (const deal of deals) {
  ...
  const createdAt = cleanStr(deal.createdAt ? deal.createdAt.toISOString() : null);
  const updatedAt = cleanStr(deal.updatedAt ? deal.updatedAt.toISOString() : null);
  ...
  const bitrixCreatedAt = cleanStr(deal.bitrixCreatedAt ? deal.bitrixCreatedAt.toISOString() : null);
  ...
  const chunk_text = [description, location, industry].filter(Boolean).join(" | ");
  ...
  record.confidence_business_strategy = inferred.confidence_scores.business_strategy;
